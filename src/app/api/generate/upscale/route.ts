...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      // P4d — via Provider Registry → PiAPI adapter → client. Args idênticos.
      // canonicalId = identidade canônica ESTÁVEL ("upscale"); requestId =
      // correlação da execução (separados, padrão pós P4c-fix).
      const providerTask = {
        canonicalId: "upscale",
        requestId,
        type: "image",
        ...
        params: {
          call: {
            op: "toolkit",
            args: { taskType: "upscale", input: { image: imageUrl, scale, face_enhance: faceEnhance } },
          },
        },
      } satisfies GenTask;
