...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      // P4a/P4c — via Provider Registry → PiAPI adapter → client. Args idênticos.
      // canonicalId = identidade canônica ESTÁVEL ("remove-bg"); requestId =
      // correlação da execução (separados após P4c-fix).
      const providerTask = {
        canonicalId: "remove-bg",
        requestId,
        type: "image",
        providerModelId: "Qubico/image-toolkit",
        input: {},
        params: {
          call: {
            op: "toolkit",
            args: { taskType: "background-remove", input: { rmbg_model: rmbgModel, image: imageUrl } },
          },
        },
      } satisfies GenTask;
      const submission = await getProvider("piapi").submit(providerTask);
