...
export async function POST(
  ...
  try {
    ...
    // §13 — INTENT IDEMPOTENCY (paridade com segment/vídeo): insert ANTES do débito;
    // unique index (user_id, generation_intent_id) → 1 geração por intenção; duplo-
    // submit/retry NÃO gera 2º débito. Compartilha webhook/inbox/P8/P7 pela tabela.
    ...
    if (genInsertErr) {
      // Duplicata de intenção → retoma SEM debitar (não cria 2º clipe/2º job).
      if ((genInsertErr as { code?: string }).code === "23505") {
        const { data: existing } = await service
          ...
          .single();
        if (existing) {
          auditLog("api.ugc.broll", "intent_resume", requestId, { generation_id: existing.id });
          return NextResponse.json({ broll: currentBroll, resumed: true, generation_id: existing.id, status: existing.status });
        }
      }
