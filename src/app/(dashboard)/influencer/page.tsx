...
export default function InfluencerPage() {
  ...
  useEffect(() => {
    try {
      if (!window.localStorage.getItem("fluxyra_influencer_onboarded")) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- sync with persisted localStorage flag on mount, not derived state.
        setOnboardOpen(true);
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    if (!selectedInfluencerId) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- clears stale content while syncing to the new selection, not derived state.
      setContentItems([]);
      return;
    }
