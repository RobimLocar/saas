...
interface ModelOpt { id: string; name: string; cost: number; modelId?: string; backend?: string; durMin?: number; durMax?: number; resolution?: string; hasAudio?: boolean; cps?: Record<string, number>; planMult?: number }
...
interface VCaps { endFrame: boolean; audio: boolean; multiShot: boolean; omni: boolean; aspects: string[]; resolutions: string[]; duration: VDur; rule1080Dur6?: boolean; avatarAudio?: boolean; motionVideo?: boolean; veoRefs?: boolean; seed?: boolean; }
...
function VideoGenNode({ id, data, selected }: NodeProps) {
  ...
  return (<NodeShell id={id} type="videoGen" title={(d.title as string) || "Video Generator"} status={status} selected={selected} runnable width={300}>
    ...
    {model ? <div className="mb-2 flex items-center justify-between text-[10px] text-[color:var(--fx-muted)]"><span>Custo estimado</span><span className="flex items-center gap-1 font-medium text-[#FBBF24]"><Zap className="h-3 w-3" fill="currentColor" />{videoGenCredits(model, caps.motionVideo ? "1080p" : resolution, caps.motionVideo ? (Number(d.motionVideoSeconds) || 5) : dur)}</span></div> : null}
