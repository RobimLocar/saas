...
import { canonicalModelId } from "@/lib/models/capabilities";
...
interface ModelOpt { id: string; name: string; cost: number; modelId?: string; backend?: string; durMin?: number; durMax?: number; resolution?: string; hasAudio?: boolean; cps?: Record<string, number>; costMap?: Record<string, Record<string, number>>; planMult?: number }
// ETAPA 2.1 Q2 — custo REAL de uma geração de vídeo (= o que o backend debita).
// cps (raw) presente → ceil(ceil(rawCps[res]×round(dur)) × planMult); igual a
// api/generate/video (effectiveCost(ceil(cps×dsafe))). Sem cps → custo flat (já
// ajustado por plano em /api/models). Retorna créditos inteiros.
function videoGenCredits(m: ModelOpt | undefined, resolution: string, duration: number): number {
  if (!m) return 0;
  if (m.cps && typeof m.cps === "object") {
    const rate = m.cps[resolution] ?? m.cps["720p"] ?? 0;
    const secs = Math.max(1, Math.round(duration || 0));
    return Math.ceil(Math.ceil(rate * secs) * (m.planMult ?? 1));
  }
  // ETAPA 7.5.1 — Hailuo cobra por (resolução, duração): mapa {res:{dur:cr}}.
  // Replica o mapeamento do adapter (768/1080 × 6/10; 1080 só com dur 6).
  if (m.costMap && typeof m.costMap === "object") {
    const hDur = Math.max(1, Math.round(duration || 0)) <= 8 ? 6 : 10;
    const hRes = resolution.includes("1080") && hDur === 6 ? "1080" : "768";
    const raw = m.costMap[hRes]?.[String(hDur)];
    if (typeof raw === "number" && raw > 0) return Math.ceil(raw * (m.planMult ?? 1));
  }
  return m.cost || 0;
}

// ETAPA 7.6.2 / P9a — resoluções REAIS por modelo de imagem. FONTE ÚNICA
// compartilhada com Studio + rota (imageResolutionOptions): só Nano Banana Pro tem
// 2K/4K; demais → "1K". Delegar evita divergência Studio/Flow.
function imgResolutions(m: ModelOpt | undefined): string[] {
  return imageResolutionOptions(m?.modelId);
}

// ETAPA 7.6.1 — custo de IMAGEM por resolução (Nano Banana Pro: mapa PLANO {res:cr}).
// Espelha a rota /api/generate/image: usa credit_cost_map[res] × plano; senão o flat.
function imageGenCredits(m: ModelOpt | undefined, resolution: string): number {
  if (!m) return 0;
  const cm = m.costMap as unknown as Record<string, number> | undefined;
  if (cm && typeof cm === "object") {
    const key = resolution === "2K" || resolution === "4K" ? resolution : "1K";
    const raw = cm[key];
    if (typeof raw === "number" && raw > 0) return Math.ceil(raw * (m.planMult ?? 1));
  }
  return m.cost || 0;
...
// ETAPA 7.2.1 — mede a duração (segundos) de um VÍDEO pela URL, para estimar o
// custo do Kling Motion (o provider cobra pela duração do vídeo de movimento).
// Best-effort no cliente; a rota mede de novo no servidor (autoritativo).
function getVideoDuration(url: string): Promise<number> {
  return new Promise((resolve) => {
    try {
      ...
      v.preload = "metadata";
      let done = false;
      const finish = (n: number) => { if (done) return; done = true; resolve(Number.isFinite(n) && n > 0 ? n : 0); };
...
function GenToolPopover({ kind, anchor, count, setCount, res, setRes, resOpts, cost, busy, onGenerate, onClose }: { kind: "angles" | "variations"; anchor: DOMRect; count: number; setCount: (f: (c: number) => number) => void; res: string; setRes: (v: string) => void; resOpts: string[]; cost: number; busy: boolean; onGenerate: () => void; onClose: () => void }) {
  ...
  return createPortal(<div ref={ref} className="fixed z-[95] flex items-center gap-2 rounded-xl fx-panel px-2.5 py-2" style={{ left: pos.left, top: pos.top, width: "max-content", minWidth: 360, maxWidth: "min(520px, calc(100vw - 32px))" }} onMouseDown={(e) => e.stopPropagation()}>
    ...
    <select value={resOpts.includes(res) ? res : resOpts[0]} onChange={(e) => setRes(e.target.value)} className="fx-ctrl h-7 w-[74px] shrink-0 appearance-none px-2 text-[11px]">{resOpts.map((r) => <option key={r} value={r}>{r}</option>)}</select>
...
function ImageGenNode({ id, data, selected }: NodeProps) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- caches the last non-empty result so the thumbnail can keep showing it while regenerating; not derived state.
  ...
  const imgModel = models.find((m) => m.id === d.model);
  const isFluxImg = Boolean(imgModel?.modelId?.startsWith("Qubico/flux")); // guidance_scale
  const isQwenImg = imgModel?.modelId === "qwen-image"; // seed/steps/flow_shift
  const imgResOpts = imgResolutions(imgModel);
  const imgRes = imgResOpts.includes((d.resolution as string)) ? (d.resolution as string) : imgResOpts[0];
  const modelCost = imageGenCredits(imgModel, imgRes);
  ...
  const cfg = (<>
    <div className="mb-2"><FModelSelect value={(d.model as string) || ""} options={models} placeholder="— modelo —" onChange={(mid) => { const nm = models.find((m) => m.id === mid); const opts = imgResolutions(nm); rf.updateNodeData(id, { model: mid, modelName: nm?.name || "", resolution: opts.includes((d.resolution as string)) ? (d.resolution as string) : opts[0] }); }} /></div>
    ...
    <div className="grid grid-cols-2 gap-2"><FSelect value={(d.aspectRatio as string) || "1:1"} onChange={(e) => rf.updateNodeData(id, { aspectRatio: e.target.value })}>{["1:1", "16:9", "9:16", "4:3", "3:4"].map((a) => <option key={a} value={a}>{a}</option>)}</FSelect><FSelect value={imgRes} onChange={(e) => rf.updateNodeData(id, { resolution: e.target.value })}>{imgResOpts.map((r) => <option key={r} value={r}>{r}</option>)}</FSelect></div>
    {/* ETAPA 7.6.3 — capabilities avançadas do schema oficial (não alteram billing). */}
    {isFluxImg ? (<div className="mb-2 flex items-center gap-2"><span className="text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Guidance</span><input type="number" min={1.5} max={5} step={0.1} value={((d.guidanceScale as string | number) ?? "") as string | number} onChange={(e) => rf.updateNodeData(id, { guidanceScale: e.target.value })} placeholder="auto (1.5–5)" className="nodrag h-7 flex-1 rounded-[8px] px-2 text-[10px]" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)", color: "var(--fx-muted)" }} /></div>) : null}
    {isQwenImg ? (<div className="mb-2 grid grid-cols-3 gap-1.5">{[["Seed", "imgSeed", "auto", undefined, undefined], ["Steps", "imgSteps", "8", 1, 16], ["Flow", "imgFlowShift", "3", 1, 7]].map(([label, key, ph, mn, mx]) => (<div key={key as string}><span className="mb-0.5 block text-[8px] uppercase tracking-wide text-[color:var(--fx-subtle)]">{label as string}</span><input type="number" min={mn as number | undefined} max={mx as number | undefined} value={((d[key as string] as string | number) ?? "") as string | number} onChange={(e) => rf.updateNodeData(id, { [key as string]: e.target.value })} placeholder={ph as string} className="nodrag h-7 w-full rounded-[8px] px-1.5 text-[10px]" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)", color: "var(--fx-muted)" }} /></div>))}</div>) : null}
  </>);
  ...
  if (vstate === "generating") {
    return (<NodeShell id={id} type="imageGen" title={title} status={status} selected={selected} noPad width={300}>
      <div className="relative overflow-hidden rounded-b-[11px]" style={{ height: genH, background: "#0a0a0b", backgroundImage: "radial-gradient(circle at 72% 14%, " + ACCENT.imageGen + "22, transparent 55%)" }}>
        {lastResult ? (<>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={lastResult} alt="" className="absolute inset-0 h-full w-full object-cover opacity-70" style={{ filter: "blur(2px)" }} /><div className="absolute inset-0" style={{ background: "rgba(0,0,0,.4)" }} /></>) : null}
  ...
  if (vstate === "result") {
    return (<NodeShell id={id} type="imageGen" title={title} status={status} selected={selected} noPad runnable={!collapsed} width={300}>
      {tool !== "none" && toolAnchor && <GenToolPopover kind={tool} anchor={toolAnchor} count={tCount} setCount={setTCount} res={tRes} setRes={setTRes} resOpts={imgResOpts} cost={imageGenCredits(imgModel, imgResOpts.includes(tRes) ? tRes : imgResOpts[0]) * tCount} busy={tBusy} onGenerate={() => genDerived(tool)} onClose={() => setTool("none")} />}
...
interface VCaps { endFrame: boolean; audio: boolean; multiShot: boolean; omni: boolean; aspects: string[]; resolutions: string[]; duration: VDur; rule1080Dur6?: boolean; avatarAudio?: boolean; motionVideo?: boolean; veoRefs?: boolean; seed?: boolean; refAudio?: boolean; refVideo?: boolean; wanExtras?: boolean; }
...
// ETAPA 7.3.1 — o schema oficial PiAPI do Veo aceita só 16:9 e 9:16 (sem 1:1).
...
const VCAPS: Record<string, VCaps> = {
  ...
  "kling-3.0-motion": { endFrame: false, audio: false, multiShot: false, omni: false, aspects: KV_ASPECTS, resolutions: ["1080p"], duration: { type: "range", min: 3, max: 30 }, motionVideo: true },
  ...
  "seedance-2.0": { endFrame: true, audio: false, multiShot: false, omni: true, aspects: SEEDANCE_ASPECTS, resolutions: ["480p", "720p", "1080p"], duration: { type: "range", min: 4, max: 15 }, refAudio: true, refVideo: true },
  "seedance-2.0-less-restriction": { endFrame: true, audio: false, multiShot: false, omni: true, aspects: SEEDANCE_ASPECTS, resolutions: ["480p", "720p", "1080p"], duration: { type: "range", min: 4, max: 15 }, refAudio: true, refVideo: true },
  "seedance-2.0-fast": { endFrame: true, audio: false, multiShot: false, omni: true, aspects: SEEDANCE_ASPECTS, resolutions: ["480p", "720p"], duration: { type: "range", min: 4, max: 15 }, refAudio: true, refVideo: true },
  "seedance-2.0-mini": { endFrame: true, audio: false, multiShot: false, omni: true, aspects: SEEDANCE_ASPECTS, resolutions: ["480p", "720p"], duration: { type: "range", min: 4, max: 15 }, refAudio: true, refVideo: true },
  "seedance-2.5": { endFrame: true, audio: false, multiShot: false, omni: true, aspects: SEEDANCE_ASPECTS, resolutions: ["480p", "720p", "1080p"], duration: { type: "range", min: 4, max: 30 }, refAudio: true, refVideo: true },
  "veo-3.1-quality": { endFrame: true, audio: true, multiShot: false, omni: false, aspects: VEO_ASPECTS, resolutions: ["720p", "1080p"], duration: { type: "enum", values: [4, 6, 8] }, veoRefs: true, seed: true },
  "veo-3.1-fast": { endFrame: true, audio: true, multiShot: false, omni: false, aspects: VEO_ASPECTS, resolutions: ["720p", "1080p"], duration: { type: "enum", values: [4, 6, 8] }, veoRefs: true, seed: true },
  "veo-3": { endFrame: false, audio: true, multiShot: false, omni: false, aspects: VEO_ASPECTS, resolutions: ["720p", "1080p"], duration: { type: "enum", values: [4, 6, 8] }, seed: true },
  "veo-3-fast": { endFrame: false, audio: true, multiShot: false, omni: false, aspects: VEO_ASPECTS, resolutions: ["720p", "1080p"], duration: { type: "enum", values: [4, 6, 8] }, seed: true },
  "hailuo": { endFrame: false, audio: false, multiShot: false, omni: false, aspects: [], resolutions: ["768p", "1080p"], duration: { type: "enum", values: [6, 10] }, rule1080Dur6: true },
  "hailuo-live": { endFrame: false, audio: false, multiShot: false, omni: false, aspects: [], resolutions: ["768p", "1080p"], duration: { type: "enum", values: [6, 10] }, rule1080Dur6: true },
  "wan-2.6": { endFrame: false, audio: true, multiShot: false, omni: false, aspects: ["16:9", "9:16", "1:1", "4:3", "3:4"], resolutions: ["720p", "1080p"], duration: { type: "enum", values: [5, 10, 15] }, wanExtras: true },
};
...
function videoCapsFor(m?: ModelOpt): VCaps {
  // P6d — normaliza aliases legados (seedance-1.5-pro / wan-2.1-video) para o id
  // canônico, para resolver a MESMA VCaps antes E depois da migration de rename.
  const cid = m?.modelId ? canonicalModelId(m.modelId) : "";
  if (cid && VCAPS[cid]) return VCAPS[cid];
...
function normalizeVideo(d: ND, caps: VCaps): ND {
  const patch: ND = {};
  let curRes = (d.resolution as string) || "";
  // P6d — alias LEGADO Hailuo: a UI passou a exibir "768p"; o default genérico de
  // node e configs antigas usam "720p". Sem este alias, "720p" (ausente das opções)
  // cairia no fallback caps.resolutions[last] = "1080p" — um UPGRADE silencioso
  // indesejado. Normaliza 720→768 quando o modelo oferece 768p mas não 720p, de modo
  // que novos saves gravem "768p" e não exista 720 em UI nova. (Billing/route/client
  // já tratam qualquer não-1080 como 768; isto só evita o salto para 1080p.)
  if (curRes === "720p" && caps.resolutions.includes("768p") && !caps.resolutions.includes("720p")) {
    curRes = "768p";
  }
...
function VideoGenNode({ id, data, selected }: NodeProps) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- caches the last non-empty start frame so the thumbnail can keep showing it while regenerating; not derived state.
    if (startFrame) setLastStart(startFrame);
  }, [startFrame]);
  ...
  if (hasEdge && srcRunning && !connImg) {
    return (<NodeShell id={id} type="videoGen" title={(d.title as string) || "Video Generator"} status={status} selected={selected} noPad width={300}>
      <div className="relative overflow-hidden" style={{ height: vmH, background: "#0a0a0b" }}>
        {lastStart ? (/* eslint-disable-next-line @next/next/no-img-element */<img src={lastStart} alt="" className="h-full w-full object-cover opacity-45" style={{ filter: "blur(2px)" }} />) : null}
  ...
  return (<NodeShell id={id} type="videoGen" title={(d.title as string) || "Video Generator"} status={status} selected={selected} runnable width={300}>
    ...
    {caps.motionVideo ? null : <div className="mb-2">{caps.duration.type === "enum" ? (<div className="flex gap-1.5">{caps.duration.values.map((v) => <button key={v} type="button" onClick={() => rf.updateNodeData(id, { duration: v })} className="nodrag h-[30px] flex-1 rounded-[8px] border text-[11px] font-medium transition" style={dur === v ? { background: ACCENT.videoGen, borderColor: ACCENT.videoGen, color: "#fff" } : { background: "var(--fx-ctrl)", borderColor: "var(--fx-border)", color: "var(--fx-muted)" }}>{v}s</button>)}</div>) : (<div className="flex items-center gap-2"><input type="range" min={caps.duration.min} max={caps.duration.max} value={dur} onChange={(e) => rf.updateNodeData(id, { duration: Number(e.target.value) })} className="nodrag flex-1" /><span className="w-8 text-right text-[10px] text-[color:var(--fx-muted)]">{dur}s</span></div>)}</div>}
    {/* ETAPA 7.2.1 — Kling Motion: vídeo de movimento (upload). O provider cobra pela duração DELE. */}
    {caps.motionVideo ? (<div className="mb-2"><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Vídeo de movimento</span>{(d.motionVideo as string) ? (<div className="flex items-center gap-2 rounded-[8px] px-2 py-1.5" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)" }}><Film className="h-3.5 w-3.5 shrink-0 text-[color:var(--fx-muted)]" /><span className="min-w-0 flex-1 truncate text-[10px] text-[color:var(--fx-muted)]">{Number(d.motionVideoSeconds) > 0 ? `${Math.ceil(Number(d.motionVideoSeconds))}s` : "vídeo carregado"}</span><button type="button" onClick={() => rf.updateNodeData(id, { motionVideo: "", motionVideoSeconds: 0 })} className="nodrag flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-black/60 text-white"><XIcon className="h-2 w-2" /></button></div>) : (<label className="fx-up nodrag flex cursor-pointer flex-col items-center justify-center gap-0.5 py-3 text-[9px]"><input type="file" accept="video/*" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; e.currentTarget.value = ""; if (!f) return; const url = await uploadFile(f); if (url) { const secs = await getVideoDuration(url); rf.updateNodeData(id, { motionVideo: url, motionVideoSeconds: secs > 0 ? Math.ceil(secs) : 0 }); } }} /><Upload className="h-3.5 w-3.5" />Vídeo de movimento (upload) — opcional; sem ele usa preset</label>)}</div>) : null}
    {/* ETAPA 7.3.2 — Veo 3.1 Reference Images (subject consistency). Slot dedicado, separado de start/end frame. Schema: 1–3, só 16:9 + 8s. */}
    {caps.veoRefs ? (<div className="mb-2"><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Reference Images <span className="normal-case text-[color:var(--fx-subtle)]">(1–3 · requer 16:9 + 8s)</span></span><div className="flex flex-wrap gap-1.5">{((d.veoRefs as string[]) || []).map((u, i) => (<div key={i} className="relative h-12 w-12 overflow-hidden rounded-md ring-1 ring-white/10">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={u} alt="" className="h-full w-full object-cover" /><button type="button" onClick={() => rf.updateNodeData(id, { veoRefs: ((d.veoRefs as string[]) || []).filter((_, j) => j !== i) })} className="nodrag absolute right-0.5 top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-black/70 text-white"><XIcon className="h-2 w-2" /></button></div>))}{(((d.veoRefs as string[]) || []).length < 3) ? (<label className="fx-up nodrag flex h-12 w-12 cursor-pointer items-center justify-center rounded-md text-[9px]"><input type="file" accept="image/*" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; e.currentTarget.value = ""; if (!f) return; const url = await uploadFile(f); if (url) rf.updateNodeData(id, { veoRefs: [...(((d.veoRefs as string[]) || [])), url].slice(0, 3) }); }} /><Plus className="h-3.5 w-3.5" /></label>) : null}</div></div>) : null}
    {/* ETAPA 7.3.2 — Veo seed (integer). Schema: só text-to-video → ignorado se houver start frame. */}
    {caps.seed ? (<div className="mb-2 flex items-center gap-2"><span className="text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Seed</span><input type="number" value={((d.seed as string | number) ?? "") as string | number} onChange={(e) => rf.updateNodeData(id, { seed: e.target.value })} placeholder="auto — só sem start frame" className="nodrag h-7 flex-1 rounded-[8px] px-2 text-[10px]" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)", color: "var(--fx-muted)" }} /></div>) : null}
    {/* ETAPA 2.1 Q2 — custo REAL (=débito): cps×duração×plano. Antes o usuário não via o preço por config. */}
    {model ? (() => { const outCr = videoGenCredits(model, caps.motionVideo ? "1080p" : resolution, caps.motionVideo ? (Number(d.motionVideoSeconds) || 5) : dur); const inSecs = caps.refVideo && Array.isArray(d.seedanceVideoSecs) ? (d.seedanceVideoSecs as number[]).reduce((a, b) => a + (b || 0), 0) : 0; const cps = model.cps ? (model.cps[resolution] ?? model.cps["720p"] ?? 0) : 0; const mult = model.planMult ?? 1; if (caps.refVideo && cps > 0 && inSecs > 0) { const total = Math.ceil(Math.ceil(cps * Math.round(dur) + (cps / 2) * inSecs) * mult); const refCr = Math.max(0, total - outCr); return (<div className="mb-2 text-[10px] text-[color:var(--fx-muted)]"><div className="flex items-center justify-between"><span>Saída ({dur}s)</span><span>{outCr}</span></div><div className="flex items-center justify-between"><span>Referência ({Math.ceil(inSecs)}s ÷ 2)</span><span>+{refCr}</span></div><div className="mt-0.5 flex items-center justify-between border-t border-[color:var(--fx-border)] pt-0.5 font-medium"><span>Total</span><span className="flex items-center gap-1 text-[#FBBF24]"><Zap className="h-3 w-3" fill="currentColor" />{total}</span></div></div>); } return (<div className="mb-2 flex items-center justify-between text-[10px] text-[color:var(--fx-muted)]"><span>Custo estimado</span><span className="flex items-center gap-1 font-medium text-[#FBBF24]"><Zap className="h-3 w-3" fill="currentColor" />{outCr}</span></div>); })() : null}
    ...
    {/* ETAPA 7.4.7.2 — Seedance Reference Video (video_urls). ≤3 · mp4/mov · o provider COBRA a duração do vídeo de entrada. Separado de start/end frame e áudio. */}
    {caps.refVideo ? (() => { const maxIn = (model?.modelId || "").includes("2.5") ? 30 : 15.4; const list = (d.seedanceVideo as string[]) || []; const secs = (d.seedanceVideoSecs as number[]) || []; const total = secs.reduce((a, b) => a + (b || 0), 0); return (<div className="mb-2"><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Reference Video <span className="normal-case text-[color:var(--fx-subtle)]">(≤3 · mp4/mov · cobra a duração — {Math.ceil(total)}/{maxIn}s)</span></span>{list.length > 0 ? (<div className="mb-1 flex flex-col gap-1">{list.map((u, i) => (<div key={i} className="flex items-center gap-2 rounded-[8px] px-2 py-1" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)" }}><Film className="h-3 w-3 shrink-0 text-[color:var(--fx-muted)]" /><span className="min-w-0 flex-1 truncate text-[10px] text-[color:var(--fx-muted)]">{Number(secs[i]) > 0 ? `${Math.ceil(Number(secs[i]))}s` : "vídeo"}</span><button type="button" onClick={() => { const a = [...list]; const s = [...secs]; a.splice(i, 1); s.splice(i, 1); rf.updateNodeData(id, { seedanceVideo: a, seedanceVideoSecs: s }); }} className="nodrag flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-black/60 text-white"><XIcon className="h-2 w-2" /></button></div>))}</div>) : null}{list.length < 3 ? (<label className="fx-up nodrag flex cursor-pointer flex-col items-center justify-center gap-0.5 py-2 text-[9px]"><input type="file" accept="video/mp4,video/quicktime" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; e.currentTarget.value = ""; if (!f) return; const url = await uploadFile(f); if (!url) return; const s = await getVideoDuration(url); const t = secs.reduce((a, b) => a + (b || 0), 0) + (s > 0 ? s : 0); if (t > maxIn) { toast.error(`Vídeos de referência somam mais de ${maxIn}s.`); return; } rf.updateNodeData(id, { seedanceVideo: [...list, url], seedanceVideoSecs: [...secs, s > 0 ? s : 0] }); }} /><Upload className="h-3.5 w-3.5" />Vídeo de referência (upload)</label>) : null}</div>); })() : null}
    {/* ETAPA 7.4.6 — Seedance Reference Audio (audio_urls). ≤3 · ≤15s total · exige imagem. NÃO é o dubbing do Kling Avatar. */}
    {caps.refAudio ? (<div className="mb-2"><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Reference Audio <span className="normal-case text-[color:var(--fx-subtle)]">(≤3 · ≤15s total · requer imagem)</span></span>{((d.seedanceAudio as string[]) || []).length > 0 ? (<div className="mb-1 flex flex-col gap-1">{((d.seedanceAudio as string[]) || []).map((u, i) => (<div key={i} className="flex items-center gap-2 rounded-[8px] px-2 py-1" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)" }}><Music className="h-3 w-3 shrink-0 text-[color:var(--fx-muted)]" /><audio src={u} controls className="nodrag h-6 min-w-0 flex-1" /><button type="button" onClick={() => { const a = [...(((d.seedanceAudio as string[]) || []))]; const s = [...(((d.seedanceAudioSecs as number[]) || []))]; a.splice(i, 1); s.splice(i, 1); rf.updateNodeData(id, { seedanceAudio: a, seedanceAudioSecs: s }); }} className="nodrag flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-black/60 text-white"><XIcon className="h-2 w-2" /></button></div>))}</div>) : null}{(((d.seedanceAudio as string[]) || []).length < 3) ? (<label className="fx-up nodrag flex cursor-pointer flex-col items-center justify-center gap-0.5 py-2 text-[9px]"><input type="file" accept="audio/*" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; e.currentTarget.value = ""; if (!f) return; const url = await uploadFile(f); if (!url) return; const secs = await getAudioDuration(url); const cur = (d.seedanceAudioSecs as number[]) || []; const total = cur.reduce((x, y) => x + y, 0) + (secs > 0 ? secs : 0); if (total > 15) { toast.error("Áudios de referência somam mais de 15s."); return; } rf.updateNodeData(id, { seedanceAudio: [...(((d.seedanceAudio as string[]) || [])), url], seedanceAudioSecs: [...cur, secs > 0 ? secs : 0] }); }} /><Upload className="h-3.5 w-3.5" />Áudio de referência (upload)</label>) : null}</div>) : null}
    ...
    {/* ETAPA 7.5.2 — Wan: prompt_extend, shot_type (só com extend), seed e audio_url custom. */}
    {caps.wanExtras ? (() => { const ext = d.wanPromptExtend !== false; const st = (d.wanShotType as string) || "single"; return (<div className="mt-2 flex flex-col gap-2"><FSwitch label="Prompt Extend" on={ext} onToggle={() => rf.updateNodeData(id, { wanPromptExtend: !ext })} />{ext ? (<div><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Shot type</span><div className="flex gap-1.5">{["single", "multi"].map((s) => <button key={s} type="button" onClick={() => rf.updateNodeData(id, { wanShotType: s })} className="nodrag h-[28px] flex-1 rounded-[8px] border text-[11px] font-medium transition" style={st === s ? { background: ACCENT.videoGen, borderColor: ACCENT.videoGen, color: "#fff" } : { background: "var(--fx-ctrl)", borderColor: "var(--fx-border)", color: "var(--fx-muted)" }}>{s}</button>)}</div></div>) : null}<div className="flex items-center gap-2"><span className="text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Seed</span><input type="number" min={0} max={2147483647} value={((d.seed as string | number) ?? "") as string | number} onChange={(e) => rf.updateNodeData(id, { seed: e.target.value })} placeholder="auto (opcional)" className="nodrag h-7 flex-1 rounded-[8px] px-2 text-[10px]" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)", color: "var(--fx-muted)" }} /></div><div><span className="mb-1 block text-[9px] uppercase tracking-wide text-[color:var(--fx-subtle)]">Reference Audio <span className="normal-case text-[color:var(--fx-subtle)]">(wav/mp3 · trilha custom)</span></span>{(d.wanAudio as string) ? (<div className="flex items-center gap-2 rounded-[8px] px-2 py-1" style={{ background: "var(--fx-ctrl)", border: "1px solid var(--fx-border)" }}><Music className="h-3 w-3 shrink-0 text-[color:var(--fx-muted)]" /><audio src={(d.wanAudio as string)} controls className="nodrag h-6 min-w-0 flex-1" /><button type="button" onClick={() => rf.updateNodeData(id, { wanAudio: "" })} className="nodrag flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-black/60 text-white"><XIcon className="h-2 w-2" /></button></div>) : (<label className="fx-up nodrag flex cursor-pointer flex-col items-center justify-center gap-0.5 py-2 text-[9px]"><input type="file" accept="audio/mpeg,audio/wav,audio/x-wav" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; e.currentTarget.value = ""; if (!f) return; const url = await uploadFile(f); if (url) rf.updateNodeData(id, { wanAudio: url }); }} /><Upload className="h-3.5 w-3.5" />Áudio (upload) — usado como trilha</label>)}</div></div>); })() : null}
...
function UpscaleNode({ id, data, selected }: NodeProps) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- caches the last non-empty input URL so the thumbnail can keep showing it while regenerating; not derived state.
    if (inputUrl) setLastInput(inputUrl);
  }, [inputUrl]);
  const displayInput = inputUrl || lastInput;
...
// §2 — id de INTENÇÃO (UUID) por execução de node. O servidor deduplica: um retry de
// rede não vira 2 débitos/2 submits. Nova execução deliberada do node → nova intenção.
function newFlowIntent(): string {
...
function Editor() {
  ...
  // §2 — guard SÍNCRONO de execução do fluxo (antes do 1º await). setRunning é
  // assíncrono; um duplo-clique no Run dispararia runFlow 2×. O ref bloqueia o 2º.
  const runningExecRef = useRef(false);
  ...
  useEffect(() => { (async () => { const load = async (t: string) => { const r = await fetch(`/api/models?type=${t}`, { cache: "no-store" }).then((x) => x.json()).catch(() => null); return Array.isArray(r?.models) ? r.models.filter((m: { hidden_from_selector?: boolean }) => !m.hidden_from_selector).map((m: { id: string; name: string; model_id?: string; credit_cost?: number; backend?: string; dur_min?: number; dur_max?: number; resolution?: string; has_audio?: boolean; credit_per_second_raw?: Record<string, number> | null; credit_cost_map?: Record<string, Record<string, number>> | null; plan_multiplier?: number }) => ({ id: m.id, name: m.name, modelId: m.model_id || undefined, cost: typeof m.credit_cost === "number" ? m.credit_cost : 0, backend: m.backend || undefined, durMin: typeof m.dur_min === "number" ? m.dur_min : undefined, durMax: typeof m.dur_max === "number" ? m.dur_max : undefined, resolution: m.resolution || undefined, hasAudio: Boolean(m.has_audio), cps: m.credit_per_second_raw || undefined, costMap: m.credit_cost_map || undefined, planMult: typeof m.plan_multiplier === "number" ? m.plan_multiplier : 1 })) : []; };const [im, vm, am] = await Promise.all([load("image"), load("video"), load("audio")]); setImageModels(im); setVideoModels(vm); setAudioModels(am); try { const me = await fetch("/api/me", { cache: "no-store" }).then((r) => r.json()).catch(() => null); if (typeof me?.credits === "number") setCredits(me.credits); } catch { /* noop */ } })(); }, []);

  useEffect(() => { let alive = true; (async () => { setLoading(true); try { const res = await fetch(`/api/flows/${flowId}`, { cache: "no-store" }); const data = await res.json().catch(() => null); if (!res.ok) throw new Error(data?.error || "Falha ao carregar."); if (!alive) return; setName(data.flow.name || "Novo Flow"); const def = data.flow.definition || {}; setNodes(Array.isArray(def.nodes) ? def.nodes : []); const defNodes = (Array.isArray(def.nodes) ? def.nodes : []) as Node[]; const ntype = new Map(defNodes.map((n) => [n.id, n.type])); const rawEdges = (Array.isArray(def.edges) ? def.edges : []) as Edge[]; const normEdges = rawEdges.map((e) => { const st = ntype.get(e.source) || ""; const tt = ntype.get(e.target) || ""; if (tt === "videoGen" && ["imageGen", "imageAsset", "refImage", "removeBg", "upscale", "storyboard"].includes(st) && e.targetHandle !== "reference") return { ...e, targetHandle: "reference" }; return e; }); setEdges(normEdges); setSaveState("saved"); setTimeout(() => { defNodes.forEach((n) => { const dd = (n.data || {}) as ND; if (dd.__gen && !dd.__result) void resumePoll(n.id, String(dd.__gen)); }); }, 0); } catch (err) { toast.error(err instanceof Error ? err.message : "Erro ao carregar."); } finally { if (alive) setLoading(false); } })(); return () => { alive = false; }; }, [flowId, setNodes, setEdges]);

  const onConnect = useCallback((c: Connection) => {
    const srcType = rf.getNode(c.source || "")?.type || ""; const tgtType = rf.getNode(c.target || "")?.type || "";
    let targetHandle: string | null = c.targetHandle ?? null;
  ...
  const runFlow = useCallback(async (targetId?: string) => {
    // §2 — guard síncrono: um fluxo/execução por vez (evita duplo-disparo do Run).
    if (runningExecRef.current) return;
    runningExecRef.current = true;
    ...
    // §12 — planejamento via núcleo PURO testável (topo + ciclo + subgrafo de retry).
    const plan = planFlowExecution(sn as FlowNodeLite[], se as FlowEdgeLite[], targetId);
    if (!plan.ok) { runningExecRef.current = false; toast.error(plan.reason === "cycle" ? "O fluxo tem um ciclo." : "Adicione nós antes de rodar."); return; }
    const runOrder = plan.runOrder;
    ...
    try {
      for (const id of runOrder) {
        ...
        // §12 — resolução de inputs (dado cruzando as edges) via núcleo PURO testável.
        const { promptText: pText, refs, dubbingAudio } = resolveNodeInputs(id, se as FlowEdgeLite[], out);
        if (node.type === "prompt") { out.set(id, (d.text as string) || ""); continue; }
        if (node.type === "refImage") { out.set(id, (d.url as string) || ""); continue; }
        if (node.type === "output") { const v = se.filter((e) => e.target === id).map((e) => out.get(e.source)).find(Boolean) || ""; out.set(id, v); setNS(id, { __status: v ? "done" : "failed", __result: v || undefined }); continue; }
        ...
        if (node.type === "imageGen" || node.type === "videoGen") {
          ...
          const all = [...refs, ...up];
          const body: Record<string, unknown> = { prompt: fp, model_uuid: d.model, aspect_ratio: d.aspectRatio, quality: "high", resolution: d.resolution || "720p", generation_intent_id: newFlowIntent() };
          if (all.length) { body.reference_images = all; body.reference_image_url = all[0]; } if (!isImg) { body.duration = d.duration; const sf = (d.startFrame as string) || refs[0] || ""; if (sf) body.start_image_url = sf; if (d.endFrame) body.end_image_url = d.endFrame as string; body.with_audio = d.audioOn !== false; const dubA = dubbingAudio || (d.dubbingAudio as string) || ""; if (dubA) { body.dubbing_audio_url = dubA; const secs = await getAudioDuration(dubA); if (secs > 0) body.dubbing_seconds = Math.ceil(secs); } const mVid = (d.motionVideo as string) || ""; if (mVid) { body.reference_videos = [mVid]; const ms = Number(d.motionVideoSeconds) || 0; if (ms > 0) body.motion_seconds = Math.ceil(ms); } const vRefs = Array.isArray(d.veoRefs) ? (d.veoRefs as string[]).filter((u) => typeof u === "string" && u).slice(0, 3) : []; if (vRefs.length) body.reference_image_urls = vRefs; const seedStr = String(d.seed ?? "").trim(); if (seedStr !== "" && Number.isFinite(Number(seedStr))) body.seed = Math.trunc(Number(seedStr)); const sAud = Array.isArray(d.seedanceAudio) ? (d.seedanceAudio as string[]).filter((u) => typeof u === "string" && u).slice(0, 3) : []; if (sAud.length) body.reference_audios = sAud; const sVidRef = Array.isArray(d.seedanceVideo) ? (d.seedanceVideo as string[]).filter((u) => typeof u === "string" && u).slice(0, 3) : []; if (sVidRef.length) { body.reference_videos = sVidRef; const vs = Array.isArray(d.seedanceVideoSecs) ? (d.seedanceVideoSecs as number[]).slice(0, 3).map((x) => Math.ceil(Number(x) || 0)) : []; body.video_ref_seconds = vs; } if (d.wanShotType === "single" || d.wanShotType === "multi") body.shot_type = d.wanShotType; if (typeof d.wanPromptExtend === "boolean") body.prompt_extend = d.wanPromptExtend; const wAud = (d.wanAudio as string) || ""; if (wAud) body.reference_audios = [wAud]; }
          if (isImg) { const sd = String(d.imgSeed ?? "").trim(); if (sd !== "" && Number.isFinite(Number(sd))) body.seed = Math.trunc(Number(sd)); const st = String(d.imgSteps ?? "").trim(); if (st !== "" && Number.isFinite(Number(st))) body.steps = Math.trunc(Number(st)); const fs = String(d.imgFlowShift ?? "").trim(); if (fs !== "" && Number.isFinite(Number(fs))) body.flow_shift = Number(fs); const gs = String(d.guidanceScale ?? "").trim(); if (gs !== "" && Number.isFinite(Number(gs))) body.guidance_scale = Number(gs); }
    ...
    } catch (err) { toast.error(err instanceof Error ? err.message : "Erro ao executar."); } finally { setRunning(false); runningExecRef.current = false; }
