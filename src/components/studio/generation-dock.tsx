...
import { useStudioStore, type MusicMode, type AceMusicMode, type SfxDuration } from "@/stores/use-studio-store";
import { resolveAudioUISpec, type AudioUIField } from "@/lib/models/audio-ui-spec";
import { imageResolutionOptions } from "@/lib/models/image-resolutions";
import { gptImage2RawCredits } from "@/lib/models/gpt-image-pricing";
...
import { applyPlanMultiplierValue } from "@/lib/billing/plan-cost";
...
interface ApiModel {
  ...
  badge?: string;
  // COMPETITOR-VIDEO-02 — badges/modes vêm da metadata do catálogo (fonte da verdade).
  badges?: string[] | null;
  modes?: string[] | null;
  ...
  // MODEL-PROPERTIES — capabilities por model+mode (consumidas pelo resolver).
  ...
  supported_params_by_mode?: Record<string, string[]> | null;
  // §7 — capacidade de Subjects/elements POR MODE (não por nome do modelo).
  capabilities_by_mode?: Record<string, { elements?: boolean } | null> | null;
  // §8 — elements travado até quote provar billing (dormant). false ⇒ editor desativado.
  elements_billing_verified?: boolean;
  pricing?: {
    mode: string;
    base_credits_per_kchar: number;
...
// P5e-fix — rótulos VISUAIS por mode id. Os IDs de modo válidos vêm da
// AudioUISpec (audioSpec.modes); este mapa é apenas apresentação (label/i18n),
// não uma segunda fonte de verdade de quais modos existem.
...
// P5f — rótulos VISUAIS dos modos ACE-Step Music. IDs vêm da AudioUISpec.
const ACE_MUSIC_MODE_LABELS: Record<string, string> = {
  instrumental: "Instrumental",
  lyrics: "With Lyrics",
};

/** Resolve a condição semântica `visibleWhen` de um campo da AudioUISpec. */
function isAudioFieldVisible(
  field: AudioUIField | undefined,
  currentMode: string
): boolean {
  if (!field) return false;
  if (!field.visibleWhen) return true;
  return field.visibleWhen.modes.includes(currentMode);
}
...
// §2 — id de INTENÇÃO (UUID) por clique/job. O servidor usa como chave de
// idempotência (mesma intenção → mesma generation). Fallback simples se o browser
// não expuser randomUUID (o servidor gera o seu próprio se o valor não for UUID).
...
// COMPETITOR-VIDEO-03 — rótulos amigáveis dos modos de vídeo (Fluxyra).
const VIDEO_MODE_LABELS: Record<string, string> = {
  "text-to-video": "Texto",
  "image-to-video": "Imagem",
  "reference-to-video": "Referência",
  "start-end-frame": "Start / End",
  "video-edit": "Editar",
  "video-extend": "Estender",
  omni: "Omni",
...
function ModelMenu({
  ...
  function capabilityTags(model: ApiModel): string[] {
    ...
    // COMPETITOR-VIDEO-02 — badges REAIS do catálogo (NEW/FAST/TURBO/MULTI/AUDIO);
    // nunca por marketing. Fallback à heurística antiga só se não houver metadata.
    if (Array.isArray(model.badges) && model.badges.length > 0) {
      // AUDIO já é indicado pelo ícone Volume2; evita duplicar.
      return model.badges.filter((b) => b !== "AUDIO");
    }
  ...
  useEffect(() => {
    ...
    if (!stillValid) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- keeps the selected family valid as the available model list changes.
...
/** §4 — lista estruturada de URLs de referência (imagens/vídeos) de um subject. */
function SubjectUrlList({
  label,
  urls,
  disabled,
  onAdd,
  ...
  return (
    <div>
      <p className="mb-1 text-[10px] font-semibold uppercase tracking-widest text-[#666666]">{label}</p>
      {urls.length > 0 && (
        <ul className="mb-1 space-y-1">
          {urls.map((u, i) => (
            <li key={i} className="flex items-center justify-between gap-2 rounded-md border border-[#242428] bg-[#101012] px-2 py-1">
              <span className="truncate text-[11px] text-[#8b8b93]">{u}</span>
              <button type="button" disabled={disabled} onClick={() => onRemove(i)} className="shrink-0 text-xs leading-none text-[#444444] hover:text-red-400 disabled:opacity-40">✕</button>
            </li>
          ))}
        </ul>
      )}
      <div className="flex gap-1.5">
        <input
          ...
          placeholder="https://…"
          className="flex-1 rounded-lg border border-[#2A2A2A] bg-[#101012] px-2.5 py-1.5 text-xs text-[#F5F5F5] outline-none placeholder:text-[#444444] focus:border-[#444444] disabled:opacity-40"
        />
        <button type="button" disabled={disabled} onClick={add} className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-2.5 py-1.5 text-xs text-[#d0d0d0] transition-colors hover:bg-white/5 disabled:opacity-40">Add</button>
      </div>
    </div>
  );
}
...
export function GenerationDock() {
  ...
  const musicMode = useStudioStore((s) => s.musicMode);
  const setMusicMode = useStudioStore((s) => s.setMusicMode);
  const lyrics = useStudioStore((s) => s.lyrics);
  const setLyrics = useStudioStore((s) => s.setLyrics);
  const negativeTags = useStudioStore((s) => s.negativeTags);
  const setNegativeTags = useStudioStore((s) => s.setNegativeTags);
  const seed = useStudioStore((s) => s.seed);
  const setSeed = useStudioStore((s) => s.setSeed);
  const aceMusicMode = useStudioStore((s) => s.aceMusicMode);
  const setAceMusicMode = useStudioStore((s) => s.setAceMusicMode);
  const aceNegativePrompt = useStudioStore((s) => s.aceNegativePrompt);
  const setAceNegativePrompt = useStudioStore((s) => s.setAceNegativePrompt);
  const sfxDuration = useStudioStore((s) => s.sfxDuration);
  const setSfxDuration = useStudioStore((s) => s.setSfxDuration);
  const mmaudioVideoUrl = useStudioStore((s) => s.mmaudioVideoUrl);
  const setMmaudioVideoUrl = useStudioStore((s) => s.setMmaudioVideoUrl);
  const mmaudioVideoDuration = useStudioStore((s) => s.mmaudioVideoDuration);
  const setMmaudioVideoDuration = useStudioStore((s) => s.setMmaudioVideoDuration);
  const mmaudioNegativePrompt = useStudioStore((s) => s.mmaudioNegativePrompt);
  const setMmaudioNegativePrompt = useStudioStore((s) => s.setMmaudioNegativePrompt);
  ...
  // GROWTH-02 — teste grátis (1 imagem global por e-mail). Estado vem do backend.
  const [imageTrialAvailable, setImageTrialAvailable] = useState(false);
  // COMPETITOR-VIDEO-03 — modo do vídeo (text/image/reference/start-end). Vem de model.modes.
  const [videoMode, setVideoMode] = useState<string>("text-to-video");
  ...
  // MODEL-PROPERTIES — valores dos params AVANÇADOS (seed/camera_fixed/prompt_expansion…),
  // model+mode-aware. Resetados ao trocar de modelo/modo (não vazam entre modelos).
  ...
  // §4/§5 — SUBJECTS/ELEMENTS (Kling O3/4K). Editor estruturado (sem JSON cru). Cada
  // subject é inline (image_refer/video_refer) OU existente (element_id) — exclusivos.
  interface SubjectDraft {
    kind: "inline" | "existing";
  ...
  const emptySubject = (): SubjectDraft => ({
    kind: "inline", reference_type: "image_refer", element_name: "", element_description: "",
    frontal_image: "", refer_images: [], refer_videos: [], element_id: "",
  });
  ...
  const mmaudioVideoInputRef = useRef<HTMLInputElement>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const overlayRef = useRef<HTMLDivElement>(null);
  // §2 — guard SÍNCRONO de double-submit (antes do 1º await). setLoading é assíncrono
  // e um duplo-clique rápido dispara handleGenerate 2× antes do re-render; o ref
  // bloqueia o 2º imediatamente. O servidor continua sendo a autoridade (intent id).
  ...
  const loadCredits = useCallback(async () => {
    try {
      ...
      setImageTrialAvailable(data.image_trial_available === true);
    } catch {
      // silencioso
    }
  }, []);
  ...
  // GROWTH-03 — retorno do checkout de recarga (success_url = /studio?topup=true).
  // Confirma a compra, atualiza o saldo e limpa a URL. O crédito em si é creditado
  // pelo webhook do Stripe (billing core, não tocado aqui).
  useEffect(() => {
    if (typeof window === "undefined") return;
    const params = new URLSearchParams(window.location.search);
    if (params.get("topup") !== "true") return;
    const timer = setTimeout(() => {
      toast.success("Compra confirmada! Seus créditos já estão sendo adicionados.");
      void loadCredits();
      params.delete("topup");
  ...
  useEffect(() => {
    ...
    // default selection: primeiro modelo VISÍVEL (não expõe legacy oculto).
    const firstVisible = models.find((m) => !m.hidden_from_selector) ?? models[0];
    if (!exists) setSelectedModelId(firstVisible.id);
  ...
  // COMPETITOR-VIDEO-03 — modos do modelo selecionado + modo efetivo (derivado, sem
  // effect): se o modo atual não existe no modelo, cai no primeiro modo suportado.
  const videoModes = useMemo(
    () => (Array.isArray(selectedModel?.modes) ? (selectedModel!.modes as string[]) : []),
    [selectedModel]
  );
  const effectiveVideoMode =
    videoModes.length === 0
      ? "text-to-video"
      : videoModes.includes(videoMode)
        ? videoMode
        : videoModes[0];

  // MODEL-PROPERTIES — verdade de capabilities do model+mode (metadata > fallback
  // por backend). A UI (resolução/aspect/duração/áudio/refs) e o servidor (isValidCombo)
  // consomem a MESMA fonte. PiAPI é preservado byte-a-byte pelo fallback do resolver.
  const videoCaps = useMemo(
    () =>
      selectedModel
        ? resolveVideoCapabilities(selectedModel as VideoModelForCaps, effectiveVideoMode, duration)
        : null,
    [selectedModel, effectiveVideoMode, duration]
  );

  // MODEL-PROPERTIES — spec dos params avançados que o model+mode suporta (metadata).
  // voice_ids É renderizado no painel como chips dos presets oficiais (GROK_VOICE_IDS),
  // seleção máx. 3; multi_shot tem UI dedicada (editor de shots). NOTA: `elements`
  // (subjects O3) é validado/dispatchado no servidor, mas o editor de subjects no dock
  // ainda NÃO existe — o cliente não envia `elements` (UI pendente; QA visual separado).
  const advancedSpec = useMemo<AdvancedParamSpec[]>(() => {
    if (activeTab !== "video" || !selectedModel) return [];
    return advancedParamsSpec(selectedModel.supported_params_by_mode ?? null, null, effectiveVideoMode);
  }, [activeTab, selectedModel, effectiveVideoMode]);
  // Ids válidos p/ o model+mode atual — usado para NÃO serializar/renderizar valores
  // avançados de outro modelo (sem reset via effect; evita set-state-in-effect).
  const advancedAllowedIds = useMemo(() => new Set(advancedSpec.map((s) => s.id)), [advancedSpec]);

  const familyGroups = useMemo<FamilyGroup[]>(() => {
    const groups: FamilyGroup[] = [];
    for (const model of models) {
      // VIDEO-RELIABILITY-01 — legacy oculto: fica em `models` (resolvível por id p/
      // Flow salvo), mas NÃO aparece no seletor de família.
      if (model.hidden_from_selector) continue;
  ...
  const aspectOptions = useMemo(() => {
    ...
    if (!selectedModel || !videoCaps) return ASPECT_RATIOS.video;
    // MODEL-PROPERTIES — verdade única (metadata > fallback backend). PiAPI idêntico.
    return videoCaps.aspectRatios;
  }, [activeTab, selectedModel, videoCaps]);

  // Resoluções suportadas variam por backend de vídeo.
  const resolutionOptions = useMemo(() => {
    // P9a — Imagem: fonte única (só Nano Banana Pro tem 2K/4K; demais → 1K).
    // Remove os controles falsos 2K/4K dos modelos que não os suportam.
    if (activeTab === "image") return imageResolutionOptions(selectedModel?.model_id);
    if (activeTab !== "video") return RESOLUTIONS.image;
    if (!selectedModel || !videoCaps) return RESOLUTIONS.video;
    // MODEL-PROPERTIES — resoluções vêm da verdade única do resolver (metadata por
    // model+mode; fallback backend preserva PiAPI incl. a regra Hailuo 1080p-só-6s,
    // Avatar/Motion mantêm o comportamento atual pelo fallback). Ver video-capabilities.ts.
    return videoCaps.resolutions;
  }, [activeTab, selectedModel, videoCaps]);
  ...
  const safeResolution = resolutionOptions.includes(resolution)
    ? resolution
    : resolutionOptions[0];
  // P9b — GPT Image 2 tem cobrança DINÂMICA (tamanho + refs). A estimativa da UI
  // usa a MESMA fonte do servidor (gptImage2RawCredits) e aplica o multiplicador do
  // plano UMA vez (applyPlanMultiplierValue) — igual ao que o servidor cobra. Demais
  // modelos mantêm o credit_cost (já ajustado por plano em /api/models).
  const imageRefCount =
    referenceImages.length > 0 ? referenceImages.length : referenceImageUrl ? 1 : 0;
  const isGptImageSelected =
    activeTab === "image" && selectedModel?.model_id === "gpt-image-2";
  const creditCost = isGptImageSelected
    ? applyPlanMultiplierValue(
        gptImage2RawCredits({ aspectRatio: safeAspect, referenceCount: imageRefCount }),
        selectedModel?.plan_multiplier ?? 1
      )
    : selectedModel?.credit_cost ?? 1;

  // P5d1b — TTS por caractere: contador + estimativa usando a MESMA matemática
  // do servidor (countTtsCharacters + calculateTtsBaseCredits + multiplicador do
  // plano). RATE (base_credits_per_kchar) e limite vêm de /api/models (catálogo).
  const { isTtsSelected, ttsCharCount, ttsMaxChars, ttsOverLimit, ttsEstimatedCost } =
    useMemo(() => {
      const isTts = activeTab === "audio" && selectedModel?.kind === "tts";
      const pricing = isTts ? selectedModel?.pricing ?? null : null;
      const charCount = isTts ? countTtsCharacters(prompt) : 0;
      const maxChars = pricing?.max_characters ?? TTS_MAX_CHARACTERS;
      const over = isTts && charCount > maxChars;
      const est =
        pricing && pricing.base_credits_per_kchar > 0 && charCount > 0
          ? applyPlanMultiplierValue(
              calculateTtsBaseCredits(charCount, pricing.base_credits_per_kchar),
              selectedModel?.plan_multiplier ?? 1
            )
          : null;
      return {
        isTtsSelected: isTts,
        ttsCharCount: charCount,
        ttsMaxChars: maxChars,
        ttsOverLimit: over,
        ttsEstimatedCost: est,
      };
    }, [activeTab, selectedModel, prompt]);

  const totalCost =
    activeTab === "image"
      ? creditCost * batchCount
      : isTtsSelected && ttsEstimatedCost != null
        ? ttsEstimatedCost
        : creditCost;
  const belowBalance = credits !== null && credits < totalCost;
  // GROWTH-02 — teste grátis cobre exatamente 1 imagem (batch 1). Nesse caso o
  // usuário NÃO está "sem créditos": pode gerar (o servidor usa Nano Banana grátis).
  const trialEligible =
    activeTab === "image" && imageTrialAvailable && batchCount === 1 && belowBalance;
  const insufficient = belowBalance && !trialEligible;

  // P5e-fix — a UI de áudio deriva da AudioUISpec (fonte única, P5c), não de um
  // hardcode de backend no JSX. O resolver traduz os dados reais do modelo
  // (model_id/kind/backend) em profile + modos + campos condicionais.
  const audioSpec = useMemo(
    () =>
      resolveAudioUISpec({
        modelId: selectedModel?.model_id ?? null,
        kind: selectedModel?.kind ?? null,
        backend: selectedModel?.backend ?? null,
      }),
    [selectedModel]
  );
  const isUdioSelected = activeTab === "audio" && audioSpec.profile === "udio";
  const audioModes = audioSpec.modes ?? [];
  const lyricsField = audioSpec.fields.find((f) => f.id === "lyrics");
  const showLyricsField = isUdioSelected && isAudioFieldVisible(lyricsField, musicMode);
  const hasNegativeTagsField = audioSpec.fields.some((f) => f.id === "negativeTags");
  const hasSeedField = audioSpec.fields.some((f) => f.id === "seed");

  // P5f — ACE-Step Music (Qubico/ace-step, kind music) via a MESMA AudioUISpec.
  // Distinto de SFX (ace-step-sfx): o profile "ace-step-music" só resolve para
  // kind music. Modos vêm de audioSpec.modes; lyrics respeita visibleWhen;
  // Negative Prompt aparece só se o field existir na spec.
  const isAceMusicSelected =
    activeTab === "audio" && audioSpec.profile === "ace-step-music";
  const showAceLyricsField =
    isAceMusicSelected && isAudioFieldVisible(lyricsField, aceMusicMode);
  const hasNegativePromptField = audioSpec.fields.some((f) => f.id === "negativePrompt");

  // P5g — Kling Sound SFX via a MESMA AudioUISpec. Duration (5|10) é BASIC/required.
  const isKlingSfxSelected =
    activeTab === "audio" && audioSpec.profile === "kling-sfx";

  // P5h — MMAudio real (video2audio) via a MESMA AudioUISpec. Video required;
  // duração client-side é só UX (limite 30s do provider). Profile dormente até o
  // SKU real existir (nenhum modelo ativo resolve para mmaudio-video2audio hoje).
  const isMmaudioSelected =
    activeTab === "audio" && audioSpec.profile === "mmaudio-video2audio";
  const mmaudioOverDuration =
    mmaudioVideoDuration != null && mmaudioVideoDuration > 30;
  ...
  const durationSnapValues = useMemo<number[] | null>(() => {
    if (activeTab !== "video" || !selectedModel || !videoCaps) return null;
    // MODEL-PROPERTIES — durações da verdade única (enum por model; regra por resolução
    // "1080p only 8s" via duration_by_resolution). Fallback backend preserva PiAPI.
    const dur = durationsForResolution(videoCaps, selectedModel as VideoModelForCaps, resolution);
    return dur.kind === "enum" ? dur.values ?? null : null; // range → slider livre
  }, [activeTab, selectedModel, videoCaps, resolution]);
  ...
  const supportsMultiShot = useMemo(() => {
    if (!selectedModel || activeTab !== "video") return false;
    // MODEL-PROPERTIES — multi-shot é uma capability ESPECIAL: só quando o endpoint
    // prova. Para Atlas (metadata é a verdade) usa capabilities.special; NUNCA a
    // heurística de nome/duração (evita controle FALSO, ex.: Kling 3.0 4K sem multi-shot).
    // PiAPI mantém a heurística histórica (sem regressão no estado atual do catálogo).
    if ((selectedModel.runtime_provider as string | undefined) === "atlas") {
      return videoCaps?.special.includes("multi-shot") ?? false;
    }
    const name = selectedModel.name.toLowerCase();
    return (
  ...
  }, [selectedModel, activeTab, videoCaps]);

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- turns multi-shot off if the selected model loses the capability.
    if (!supportsMultiShot && multiShotEnabled) setMultiShotEnabled(false);
  }, [supportsMultiShot, multiShotEnabled]);

  // §7 — Subjects/elements aparecem por CAPABILITY (capabilities_by_mode[mode].elements),
  // NUNCA por nome do modelo. §8 — só habilitados quando o billing (quote) provar paridade
  // (elements_billing_verified). Enquanto false ⇒ editor renderiza TRAVADO e NÃO serializa.
  const elementsCapableMode = useMemo(() => {
    if (!selectedModel || activeTab !== "video") return false;
    if ((selectedModel.runtime_provider as string | undefined) !== "atlas") return false;
    const cap = selectedModel.capabilities_by_mode?.[effectiveVideoMode];
    return !!cap && cap.elements === true;
  }, [selectedModel, activeTab, effectiveVideoMode]);
  const elementsUnlocked = elementsCapableMode && selectedModel?.elements_billing_verified === true;

  // Constrói o array `elements` no SHAPE OFICIAL (image_refer/video_refer | element_id).
  // Descarta subjects vazios; NÃO envia inline+element_id juntos (mutuamente exclusivos).
  function buildSubjects(): Array<Record<string, unknown>> {
    const out: Array<Record<string, unknown>> = [];
    for (const s of subjects) {
      if (s.kind === "existing") {
        if (s.element_id.trim()) out.push({ reference_type: s.reference_type, element_id: s.element_id.trim() });
        continue;
      }
      const base: Record<string, unknown> = { reference_type: s.reference_type };
      if (s.element_name.trim()) base.element_name = s.element_name.trim();
      if (s.element_description.trim()) base.element_description = s.element_description.trim();
      if (s.reference_type === "image_refer") {
        if (s.frontal_image.trim()) base.frontal_image = s.frontal_image.trim();
        if (s.refer_images.length) base.refer_images = s.refer_images;
        if (!base.frontal_image && !s.refer_images.length) continue;
      } else {
        if (s.refer_videos.length) base.refer_videos = s.refer_videos; else continue;
      }
      out.push(base);
    }
    return out;
  ...
  // P6b — trava AUTORITATIVA do multishot na UI: count ≤ 6 E soma ≤ 15s. Calcula
  // sobre os shots que serão DE FATO enviados (buildShots), cobrindo os dois modos
  // (auto: count×perShot; custom: soma dos preenchidos). O servidor também rejeita
  // (validateVideoRequest) — aqui é UX para não deixar gerar/debitar.
  const multiShotActive = multiShotEnabled && supportsMultiShot;
  const multiShotShots = multiShotActive ? buildShots() : [];
  const multiShotTotalSeconds = multiShotShots.reduce(
    (sum, s) => sum + (Number(s.duration) || 0),
    0
  );
  const multiShotOverLimit =
    multiShotActive &&
    (multiShotShots.length > 6 || multiShotTotalSeconds > 15);
  ...
  // P5h — MMAudio: seleciona UM vídeo. Lê a duração via metadata do browser (só UX);
  // se > 30s, bloqueia sem upload (limite do provider). Caso ok, envia via o MESMO
  // /api/upload (bucket público) e guarda a URL pública. Falha de upload não seta a URL.
  async function handleMmaudioVideoPick(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    // Duração client-side (UX apenas; NÃO é autoridade de billing).
    ...
    if (durationSec != null && durationSec > 30) {
      toast.error("MMAudio supports videos up to 30 seconds.");
      return;
    }
    const toastId = toast.loading(t("uploadingFile"));
    try {
      const url = await uploadReferenceFile(file);
      setMmaudioVideoUrl(url);
      setMmaudioVideoDuration(durationSec);
      toast.success(t("referenceAdded"), { id: toastId });
    } catch {
      toast.error(t("cantSendFiles"), { id: toastId });
    }
  ...
  async function submitSingleGeneration(parentRequestId?: string, intentId?: string) {
    ...
    // §2 — idempotency de intenção: mesma intenção (duplo-clique/retry) = mesma
    // generation no servidor (1 débito / 1 submit). Cada job de batch tem intent própria.
    if (intentId) body.generation_intent_id = intentId;
    ...
    // P5e — Udio (music-u): modo semântico + letra custom + avançado. O backend
    // mapeia music_mode → lyrics_type. A letra NÃO é traduzida (texto do usuário).
    if (activeTab === "audio" && isUdioSelected) {
      body.music_mode = musicMode;
      if (musicMode === "custom-lyrics" && lyrics.trim()) {
        body.lyrics = lyrics; // preserva quebras de linha ([Verse]/[Chorus])
      }
      if (negativeTags.trim()) body.negative_tags = negativeTags.trim();
      const seedNum = parseInt(seed.trim(), 10);
      if (seed.trim() && Number.isFinite(seedNum)) body.seed = seedNum;
    }

    // P5f — ACE-Step Music (Qubico/ace-step): modo + letra (só lyrics) + negative
    // prompt. instrumental NÃO envia lyrics (route resolve "[inst]"). Letra e
    // negative prompt NÃO são traduzidos. Campos só saem quando o profile é
    // ace-step-music (sem vazamento p/ Udio/TTS/SFX).
    if (activeTab === "audio" && isAceMusicSelected) {
      body.music_mode = aceMusicMode;
      if (aceMusicMode === "lyrics" && lyrics.trim()) {
        body.lyrics = lyrics; // preserva quebras/tags [verse]/[chorus]
      }
      if (aceNegativePrompt.trim()) body.negative_prompt = aceNegativePrompt.trim();
    }

    // P5g — Kling Sound SFX: envia duration (5|10). Campo só sai quando o profile
    // é kling-sfx (sem vazar para Udio/ACE/TTS/SFX-alias).
    if (activeTab === "audio" && isKlingSfxSelected) {
      body.duration = sfxDuration;
    }

    // P5h — MMAudio: envia video_url + negative_prompt. Campos só saem quando o
    // profile é mmaudio (sem vazar para Kling/ACE/Udio/TTS/SFX-alias).
    if (activeTab === "audio" && isMmaudioSelected) {
      if (mmaudioVideoUrl) body.video_url = mmaudioVideoUrl;
      if (mmaudioNegativePrompt.trim()) {
        body.negative_prompt = mmaudioNegativePrompt.trim();
      }
    }
    ...
    if (activeTab === "video") {
      ...
      if (multiShotEnabled && supportsMultiShot) {
        ...
        if ((selectedModel?.runtime_provider as string | undefined) === "atlas") {
          // Kling 4K/O3 (Atlas): multi_shot custom → multi_prompt[{prompt,duration}].
          // O servidor revalida (soma==duração, ≤6, cada≥1) e monta o payload exato.
          body.multi_shot = true;
          body.shot_type = "customize";
          body.multi_prompt = shots;
        } else {
          body.shots = shots; // PiAPI (formato histórico) — inalterado.
        }
      }
      // §8 — elements SÓ serializa quando a capacidade está DESTRAVADA por billing
      // (elements_billing_verified). Enquanto dormant, nada é enviado (billing-safe) e
      // o servidor tampouco aceita (fora de supported_params_by_mode). Sem vazamento a
      // outros modelos: elementsCapableMode é por model+mode.
      if (elementsUnlocked) {
        const els = buildSubjects();
        if (els.length > 0) body.elements = els;
      }
      // MODEL-PROPERTIES — with_audio segue o audio_mode do modelo (always→on, none→off,
      // toggle→escolha do usuário). Evita enviar toggle a um modelo que não suporta.
      body.with_audio =
        videoCaps?.audio === "always" ? true : videoCaps?.audio === "none" ? false : audioEnabled;
      // COMPETITOR-VIDEO-03 — modo (server valida ∈ model.modes e escolhe o atlas_model confiável).
      if (videoModes.length > 0) body.mode = effectiveVideoMode;

      // MODEL-PROPERTIES — params AVANÇADOS do model+mode atual (seed/camera_fixed/
      // prompt_expansion…). Só os permitidos p/ este model+mode entram (advancedAllowedIds)
      // — valores de outro modelo NUNCA são serializados. O servidor revalida por metadata.
      for (const [k, v] of Object.entries(advancedValues)) {
        if (!advancedAllowedIds.has(k) || v === undefined || v === null || v === "") continue;
        body[k] = v;
      }
    }

    // §9 — MODE SERIALIZER: remove campos de mídia que não pertencem ao modo ativo
    // (ex.: refs de um modo Reference não vazam ao trocar para Text). Só afeta vídeo
    // com modos declarados; o servidor revalida (isValidCombo) de qualquer forma.
    if (activeTab === "video" && typeof body.mode === "string") {
      const serialized = serializeVideoRequestForMode(body, body.mode as string);
      for (const k of Object.keys(body)) if (!(k in serialized)) delete body[k];
      Object.assign(body, serialized);
    }
  ...
  async function handleGenerate() {
    ...
    // P5d1b — TTS: bloqueia acima do limite oficial de caracteres (o servidor
    // também rejeita com 400; aqui é só UX para evitar débito/insert).
    if (ttsOverLimit) {
      genLog("handleGenerate.bloqueado", requestId, { motivo: "tts_acima_do_limite", chars: ttsCharCount, max: ttsMaxChars }, performance.now() - t0);
      toast.error(`Texto muito longo: ${ttsCharCount} / ${ttsMaxChars} caracteres.`);
      return;
    }

    // P6b — Multishot: bloqueia count > 6 OU soma > 15s (o servidor também rejeita
    // com 400; aqui é UX para evitar débito/insert de uma task que falharia).
    if (multiShotOverLimit) {
      genLog("handleGenerate.bloqueado", requestId, {
        motivo: "multishot_acima_do_limite",
        shots: multiShotShots.length,
        total_s: multiShotTotalSeconds,
      }, performance.now() - t0);
      toast.error(
        multiShotShots.length > 6
          ? `Máximo de 6 shots (atual: ${multiShotShots.length}).`
          : `Duração total dos shots ${multiShotTotalSeconds}s excede 15s.`
      );
      return;
    }

    // P5h — MMAudio: exige vídeo de origem e ≤ 30s (UX; o servidor também valida).
    if (isMmaudioSelected && !mmaudioVideoUrl) {
      genLog("handleGenerate.bloqueado", requestId, { motivo: "mmaudio_sem_video" }, performance.now() - t0);
      toast.error("Selecione um vídeo de origem para o MMAudio.");
      return;
    }
    if (isMmaudioSelected && mmaudioOverDuration) {
      genLog("handleGenerate.bloqueado", requestId, { motivo: "mmaudio_video_longo" }, performance.now() - t0);
      toast.error("MMAudio supports videos up to 30 seconds.");
      return;
    }
    ...
    // §2 — GUARD SÍNCRONO de double-submit. Todas as validações acima são síncronas
    // (sem await), então posicioná-lo aqui — imediatamente antes do 1º trabalho
    // assíncrono — garante que um duplo-clique só avance UMA vez. Liberado no finally;
    // um novo Generate deliberado gera nova intenção. Servidor = autoridade final.
    ...
    try {
      if (activeTab === "image" && batchCount > 1) {
        // Batch: cada job é uma geração DISTINTA → intenção própria por job.
        const jobs = Array.from({ length: batchCount }).map(() =>
          submitSingleGeneration(requestId, cryptoRandomId())
        );
        ...
        await submitSingleGeneration(requestId, cryptoRandomId());
      ...
      setLoading(false);
      generatingRef.current = false; // libera o guard p/ um novo Generate deliberado
    }
  }
  ...
  return (
    <section className="fixed bottom-4 left-1/2 z-30 mx-auto w-full max-w-[1100px] min-w-0 -translate-x-1/2 rounded-2xl border border-[#242428] bg-[#141416] px-4 py-4 shadow-[0_-8px_40px_rgba(0,0,0,0.4)] backdrop-blur-sm">
      ...
      >
        ...
        {/* Udio (music-u) — modo + letra custom + avançado (P5e) */}
        {activeTab === "audio" && isUdioSelected && (
          <div className="mb-3 space-y-2">
            {/* Seletor de modo — IDs vêm da AudioUISpec (audioModes); os rótulos
                são apenas apresentação (MUSIC_MODE_LABELS). */}
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] p-1">
              {audioModes.map((m) => (
                <button
                  key={m.id}
                  type="button"
                  onClick={() => setMusicMode(m.id as MusicMode)}
                  className={cn(
                    "h-9 rounded-lg text-xs font-medium transition-colors duration-150",
                    musicMode === m.id
                      ? "bg-[#2A2A2A] text-[#F5F5F5]"
                      : "text-[#888888] hover:text-[#F5F5F5]"
                  )}
                >
                  {MUSIC_MODE_LABELS[m.id] ?? m.id}
                </button>
              ))}
            </div>

            {/* Letra custom — visibilidade vem do visibleWhen da spec. Não é traduzida. */}
            {showLyricsField && (
              <div>
                <label className="mb-1 block text-xs font-medium text-[#888888]">
                  Lyrics
            ...
            {/* Avançado — disponibilidade reflete os fields da spec (negativeTags/seed).
                Colapsável, como no vídeo. */}
            {!collapsed && (hasNegativeTagsField || hasSeedField) && (
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {hasNegativeTagsField && (
                  <div>
                    <label className="mb-1 block text-xs font-medium text-[#888888]">
                      Negative Tags
                    </label>
                    <input
                      ...
                      placeholder="e.g. distorted, low quality"
                      className="w-full rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] px-3 py-2 text-sm leading-5 text-[#F5F5F5] outline-none placeholder:text-[#666666] focus:border-[#3A3A3A]"
                    />
                  </div>
                )}
                {hasSeedField && (
        ...
        {/* ACE-Step Music (Qubico/ace-step, kind music) — modo + letra + negative prompt (P5f) */}
        {activeTab === "audio" && isAceMusicSelected && (
          <div className="mb-3 space-y-2">
            {/* Seletor de modo — IDs vêm da AudioUISpec (instrumental / lyrics). */}
            <div className="grid grid-cols-2 gap-1 rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] p-1">
              {audioModes.map((m) => (
                <button
                  key={m.id}
                  type="button"
                  onClick={() => setAceMusicMode(m.id as AceMusicMode)}
                  className={cn(
                    "h-9 rounded-lg text-xs font-medium transition-colors duration-150",
                    aceMusicMode === m.id
                      ? "bg-[#2A2A2A] text-[#F5F5F5]"
                ...
                >
                  {ACE_MUSIC_MODE_LABELS[m.id] ?? m.id}
                </button>
              ))}
            </div>

            {/* Letra — visibilidade via visibleWhen (modo lyrics). Não traduzida. */}
            {showAceLyricsField && (
              <div>
                <label className="mb-1 block text-xs font-medium text-[#888888]">
                  Lyrics
            ...
            {/* Avançado — Negative Prompt (só se a spec tiver o field). Colapsável. */}
            {!collapsed && hasNegativePromptField && (
              <div>
                <label className="mb-1 block text-xs font-medium text-[#888888]">
                  Negative Prompt
                </label>
                <input
                  type="text"
                  value={aceNegativePrompt}
                  maxLength={500}
                  onChange={(event) => setAceNegativePrompt(event.target.value)}
        ...
        {/* Kling Sound SFX (P5g) — Duration 5s/10s (BASIC, required) */}
        {activeTab === "audio" && isKlingSfxSelected && (
          <div className="mb-3">
            <label className="mb-1 block text-xs font-medium text-[#888888]">
              Duration
            </label>
            <div className="grid grid-cols-2 gap-1 rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] p-1">
              {([5, 10] as SfxDuration[]).map((d) => (
                <button
                  key={d}
                  type="button"
                  onClick={() => setSfxDuration(d)}
                  aria-pressed={sfxDuration === d}
                  className={cn(
                    "h-9 rounded-lg text-xs font-medium transition-colors duration-150",
                    sfxDuration === d
                      ? "bg-[#2A2A2A] text-[#F5F5F5]"
        ...
        {/* MMAudio (Qubico/mmaudio) — Source Video + Negative Prompt (P5h) */}
        {activeTab === "audio" && isMmaudioSelected && (
          <div className="mb-3 space-y-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-[#888888]">
                Source Video
              ...
              <div className="flex flex-wrap items-center gap-2">
                {mmaudioVideoUrl ? (
                  <>
                    <div className="relative flex h-9 items-center gap-2 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-3 text-xs text-[#F5F5F5]">
                      <Video className="h-3.5 w-3.5 text-[#888888]" />
                      Video
                      {mmaudioVideoDuration != null
                        ? ` · ${Math.round(mmaudioVideoDuration)}s`
                        : ""}
                      <button
                        type="button"
                        onClick={() => {
                          setMmaudioVideoUrl(null);
                          setMmaudioVideoDuration(null);
                        }}
                        className="text-[#666666] hover:text-[#F5F5F5]"
                      >
                        <X className="h-3 w-3" />
                    ...
                    <button
                      type="button"
                      onClick={() => mmaudioVideoInputRef.current?.click()}
              ...
              {mmaudioOverDuration && (
                <p className="mt-1 text-xs text-red-400">
                  MMAudio supports videos up to 30 seconds.
                </p>
              )}
            </div>
        ...
        {/* MODEL-PROPERTIES — painel AVANÇADO expandido (model+mode-aware; metadata). */}
        {!collapsed && activeTab === "video" && advancedOpen && advancedSpec.length > 0 && (
          <div className="mb-4 rounded-lg border border-[#2A2A2A] bg-[#141414] p-3">
            <div className="mb-2 text-[11px] font-medium uppercase tracking-wide text-[#888888]">Avançado</div>
            <div className="flex flex-col gap-2">
              {advancedSpec.map((spec) => (
                spec.type === "voice_ids" ? (
                  <div key={spec.id} className="flex flex-col gap-1.5">
                    <span className="text-xs text-[#c9c9d1]">{spec.label} <span className="text-[#888888]">(até {spec.max ?? 3})</span></span>
                    <div className="flex flex-wrap gap-1.5">
                      {GROK_VOICE_IDS.map((vid) => {
                        const sel = Array.isArray(advancedValues.voice_ids) && (advancedValues.voice_ids as string[]).includes(vid);
                        return (
                          <button
                            key={vid}
                            type="button"
                            onClick={() => setAdvancedValues((prev) => {
                              const cur = Array.isArray(prev.voice_ids) ? (prev.voice_ids as string[]) : [];
                              if (cur.includes(vid)) return { ...prev, voice_ids: cur.filter((x) => x !== vid) };
                              if (cur.length >= (spec.max ?? 3)) return prev;
                              return { ...prev, voice_ids: [...cur, vid] };
                            })}
                            className={cn(
                              "rounded-full border px-2 py-0.5 text-[11px] capitalize transition-colors",
                              sel ? "border-[#7C3AED]/50 bg-[#7C3AED]/20 text-[#A78BFA]" : "border-[#2A2A2A] bg-white/5 text-[#c9c9d1] hover:bg-white/10"
                            )}
                          >
                            {vid}
                          </button>
                ...
                <div key={spec.id} className="flex items-center justify-between gap-3">
                  <label htmlFor={`adv-${spec.id}`} className="text-xs text-[#c9c9d1]">{spec.label}</label>
                  {spec.type === "bool" ? (
                    <button
                      type="button"
                      id={`adv-${spec.id}`}
                      aria-pressed={advancedValues[spec.id] === true}
                      onClick={() => setAdvancedValues((prev) => ({ ...prev, [spec.id]: !(prev[spec.id] === true) }))}
                      className={cn(
                        "flex h-7 w-12 items-center rounded-full border border-[#2A2A2A] px-1 transition-colors",
                        advancedValues[spec.id] === true ? "bg-[#7C3AED]/40" : "bg-white/5"
                      )}
                    >
                      <span className={cn("h-5 w-5 rounded-full bg-white transition-transform", advancedValues[spec.id] === true ? "translate-x-5" : "translate-x-0")} />
                    </button>
                  ) : spec.type === "enum" ? (
                    <select
                      id={`adv-${spec.id}`}
                      value={typeof advancedValues[spec.id] === "string" ? (advancedValues[spec.id] as string) : (spec.options?.[0] ?? "")}
                      onChange={(e) => setAdvancedValues((prev) => ({ ...prev, [spec.id]: e.target.value }))}
                      className="h-8 w-40 rounded-md border border-[#2A2A2A] bg-[#1A1A1A] px-2 text-xs capitalize text-[#F5F5F5] outline-none focus:border-[#7C3AED]/50"
                    >
                    ...
                    <input
                      id={`adv-${spec.id}`}
                      type="number"
                      value={typeof advancedValues[spec.id] === "number" ? (advancedValues[spec.id] as number) : ""}
                      onChange={(e) => {
                        const n = e.target.value === "" ? undefined : Math.trunc(Number(e.target.value));
                        setAdvancedValues((prev) => ({ ...prev, [spec.id]: Number.isFinite(n as number) ? n : undefined }));
                      }}
                      placeholder="aleatório"
                      className="h-8 w-28 rounded-md border border-[#2A2A2A] bg-[#1A1A1A] px-2 text-xs text-[#F5F5F5] outline-none focus:border-[#7C3AED]/50"
                    />
                  ) : (
                    <input
                      id={`adv-${spec.id}`}
                      type="text"
                      value={typeof advancedValues[spec.id] === "string" ? (advancedValues[spec.id] as string) : ""}
                      onChange={(e) => setAdvancedValues((prev) => ({ ...prev, [spec.id]: e.target.value }))}
                      className="h-8 w-40 rounded-md border border-[#2A2A2A] bg-[#1A1A1A] px-2 text-xs text-[#F5F5F5] outline-none focus:border-[#7C3AED]/50"
                    />
                  )}
                </div>
                )
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p
                        className={cn(
                          "mt-1 text-[10px]",
                          autoShotCount * autoShotDuration > 15
                            ? "text-red-400"
                            : "text-[#555555]"
                        )}
                      >
                        Total: {autoShotCount * autoShotDuration}s / 15s
                        {autoShotCount * autoShotDuration > 15 &&
                          " — reduza para 15s ou menos"}
                      </p>
                    </div>
                  </div>
                )}
        ...
        {/* §4/§5/§7 — SUBJECTS/ELEMENTS (Kling O3/4K). Aparece por CAPABILITY
            (capabilities_by_mode[mode].elements), não por nome. §8 — travado até o
            quote provar billing (elements_billing_verified). Editor estruturado, sem JSON. */}
        {!collapsed && activeTab === "video" && elementsCapableMode && (
          <div className="mb-4 rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] p-3">
            <div className="mb-2 flex items-center justify-between gap-2">
              ...
              {!elementsUnlocked && (
                <span
                  title="Subjects/elements ficam disponíveis quando a verificação de billing (quote) confirmar a paridade de custo. Enquanto isso, o editor fica travado."
                  className="rounded-md border border-[#3a3320] bg-[#241f10] px-2 py-0.5 text-[10px] font-medium text-[#d8b25a]"
                >
                  Locked — billing quote required
                </span>
              )}
            </div>
            <p className="mb-2 text-[11px] leading-4 text-[#666666]">
              Adicione personagens/objetos de referência. Cada subject vira uma tag{" "}
              <code className="text-[#9F67FF]">{"<<<element_N>>>"}</code> no prompt.
            </p>

            <div className="space-y-2">
              {subjects.map((s, i) => (
                <div key={i} className="rounded-lg border border-[#242428] bg-[#141416] p-2.5">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-[11px] text-[#8b8b93]">Subject {i + 1} — {elementTag(i)}</span>
                    <button
                      type="button"
                      disabled={!elementsUnlocked}
                      onClick={() => setSubjects((prev) => prev.filter((_, j) => j !== i))}
                      className="text-sm leading-none text-[#444444] hover:text-red-400 disabled:opacity-40"
                    >
                      ✕
                    </button>
                  </div>

                  {/* Inline (novo) vs Existing (element_id) — mutuamente exclusivos */}
                  <div className="mb-2 flex gap-1">
                    {(["inline", "existing"] as const).map((k) => (
                      <button
                        key={k}
                        type="button"
                        disabled={!elementsUnlocked}
                        onClick={() => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, kind: k } : x)))}
                        className={cn(
                          "rounded-lg px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide transition-colors disabled:opacity-40",
                          s.kind === k ? "bg-[#2A2A2A] text-[#F5F5F5]" : "text-[#666666] hover:text-[#999999]"
                        )}
                      >
                        {k === "inline" ? "New subject" : "Existing"}
                      </button>
                    ))}
                  </div>
                  ...
                  {s.kind === "existing" ? (
                    <input
                      ...
                      value={s.element_id}
                      disabled={!elementsUnlocked}
                      onChange={(e) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, element_id: e.target.value } : x)))}
                      placeholder="Element ID (subject salvo)"
                      className="w-full rounded-lg border border-[#2A2A2A] bg-[#101012] px-2.5 py-1.5 text-xs text-[#F5F5F5] outline-none placeholder:text-[#444444] focus:border-[#444444] disabled:opacity-40"
                    />
                  ) : (
                    <div className="space-y-2">
                      <input
                        type="text"
                        value={s.element_name}
                        disabled={!elementsUnlocked}
                        onChange={(e) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, element_name: e.target.value } : x)))}
                        placeholder="Name (ex.: minha modelo)"
                        className="w-full rounded-lg border border-[#2A2A2A] bg-[#101012] px-2.5 py-1.5 text-xs text-[#F5F5F5] outline-none placeholder:text-[#444444] focus:border-[#444444] disabled:opacity-40"
                      />
                      <div className="flex gap-1">
                        {(["image_refer", "video_refer"] as const).map((rt) => (
                          <button
                            key={rt}
                            type="button"
                            disabled={!elementsUnlocked}
                            onClick={() => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, reference_type: rt } : x)))}
                            className={cn(
                              "rounded-lg px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide transition-colors disabled:opacity-40",
                              s.reference_type === rt ? "bg-[#2A2A2A] text-[#F5F5F5]" : "text-[#666666] hover:text-[#999999]"
                            )}
                          >
                            {rt === "image_refer" ? "Image" : "Video"}
                          </button>
                        ))}
                      </div>
                      {s.reference_type === "image_refer" ? (
                        <>
                          <input
                            type="text"
                            value={s.frontal_image}
                            disabled={!elementsUnlocked}
                            onChange={(e) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, frontal_image: e.target.value } : x)))}
                          ...
                          <SubjectUrlList
                            ...
                            urls={s.refer_images}
                            disabled={!elementsUnlocked}
                            onAdd={(u) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, refer_images: [...x.refer_images, u] } : x)))}
                            onRemove={(k) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, refer_images: x.refer_images.filter((_, z) => z !== k) } : x)))}
                          />
                        </>
                      ) : (
                        <SubjectUrlList
                          ...
                          urls={s.refer_videos}
                          disabled={!elementsUnlocked}
                          onAdd={(u) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, refer_videos: [...x.refer_videos, u] } : x)))}
                          onRemove={(k) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, refer_videos: x.refer_videos.filter((_, z) => z !== k) } : x)))}
                        />
                      )}
                      <input
                        type="text"
                        value={s.element_description}
                        disabled={!elementsUnlocked}
                        onChange={(e) => setSubjects((prev) => prev.map((x, j) => (j === i ? { ...x, element_description: e.target.value } : x)))}
                  ...
                  <button
                    type="button"
                    disabled={!elementsUnlocked}
                    onClick={() => setPrompt(prompt.trim() ? `${prompt.trim()} ${elementTag(i)}` : elementTag(i))}
                    className="mt-2 text-[11px] text-[#7C3AED] hover:text-[#9F67FF] disabled:opacity-40"
                  >
                    Insert {elementTag(i)} into prompt
                  </button>
                </div>
              ))}
              ...
              <button
                type="button"
                disabled={!elementsUnlocked || subjects.length >= (selectedModel?.max_reference_images ?? 4)}
                ...
                {videoModes.length > 1 && (
                  <div>
                    ...
                    <select
                      ...
                      className="h-9 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-2.5 text-xs text-[#F5F5F5] outline-none"
                    >
                      {videoModes.map((m) => (
                        <option key={m} value={m}>
                          {VIDEO_MODE_LABELS[m] ?? m}
                        </option>
                      ))}
                    </select>
                  </div>
        ...
        />
        {/* P5h — MMAudio: single video (não multiple) → duração check + upload */}
        <input
          ref={mmaudioVideoInputRef}
          type="file"
          accept="video/*"
          className="hidden"
          onChange={(event) => void handleMmaudioVideoPick(event)}
        />
        ...
        <div className={cn("flex min-w-0 items-center gap-2 xl:flex-nowrap", collapsed && "hidden")}>
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5 xl:flex-nowrap">
            ...
            {activeTab === "audio" && selectedModel?.kind === "tts" && (
              <>
                ...
                {/* P5d1b — contador de caracteres + estimativa de créditos (TTS). */}
                <div className="flex h-9 items-center gap-1.5 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-2.5 py-1.5 text-xs">
                  <span className={cn("text-[#888888]", ttsOverLimit && "text-red-400")}>
                    {ttsCharCount.toLocaleString()} / {ttsMaxChars.toLocaleString()}
                  </span>
                  {ttsEstimatedCost != null && !ttsOverLimit ? (
                    <span className="text-[#c9c9d1]">· ~{ttsEstimatedCost} cr</span>
                  ) : null}
                </div>
            ...
            {/* MODEL-PROPERTIES — Áudio conforme audio_mode do modelo (verdade única):
                "toggle" = botão on/off; "always" = indicador (áudio nativo sempre on,
                sem toggle falso); "none" = nenhum ícone. PiAPI preservado (resolver mapeia
                has_audio=false→none, senão→toggle). */}
            {activeTab === "video" && videoCaps?.audio === "toggle" && (
              <button
                type="button"
                onClick={() => {
            ...
            {activeTab === "video" && videoCaps?.audio === "always" && (
              <div
                title="Áudio nativo sempre ativo"
                aria-label="Áudio nativo sempre ativo"
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#2A2A2A] bg-[#7C3AED]/15 text-[#A78BFA]"
              >
                <Volume2 className="h-3.5 w-3.5" />
              </div>
            )}

            {/* MODEL-PROPERTIES — botão "Avançado" aparece SÓ quando o model+mode tem
                params avançados (metadata). A barra principal segue limpa. */}
            {activeTab === "video" && advancedSpec.length > 0 && (
              <button
                type="button"
                ...
                className={cn(
                  "flex h-8 items-center gap-1 rounded-lg border border-[#2A2A2A] px-2 text-xs transition-colors duration-150",
                  advancedOpen ? "bg-[#7C3AED]/15 text-[#A78BFA] ring-1 ring-[#7C3AED]/50" : "bg-white/5 text-[#c9c9d1] hover:bg-white/10"
                )}
              >
          ...
          <div className="flex flex-shrink-0 flex-col items-end">
            <button
              type="submit"
              disabled={loading || insufficient || multiShotOverLimit}
              className={cn(
                ...
                (loading || insufficient || multiShotOverLimit) && "cursor-not-allowed opacity-50 hover:brightness-100 hover:shadow-none"
              )}
            >
              {loading ? (
            ...
            {trialEligible && (
              <span className="mt-1 text-[11px] text-[#A78BFA]">
                Teste grátis: 1 imagem com Nano Banana.
              </span>
            )}
            {insufficient && (
              <div className="mt-1 flex flex-col items-end gap-0.5 text-[11px]">
                <span className="text-right text-[#FCA5A5]">
                  {activeTab === "image" && imageTrialAvailable === false
                    ? "Você usou sua imagem grátis. Recarregue para continuar criando imagens, vídeos, áudio e UGC."
                    : `Você precisa de créditos para continuar. Esta geração custa ${totalCost} créditos.`}
                </span>
                <a
                  href="/pricing#recarga"
                  className="font-semibold text-[#A78BFA] underline hover:text-[#C4B5FD]"
                >
                  Recarregar créditos
                </a>
              </div>
            )}
            {!insufficient && !trialEligible && multiShotOverLimit && (
              <span className="mt-1 text-[11px] text-[#FCA5A5]">
                {multiShotShots.length > 6
                  ? "Máximo de 6 shots."
                  : `Shots somam ${multiShotTotalSeconds}s (máx. 15s).`}
              </span>
            )}
          </div>
        </div>
