...
import { MultiAudioPlayer } from "@/components/studio/multi-audio-player";
import { getGenerationResultUrls } from "@/lib/media/result-urls";
...
export interface LightboxItem {
  ...
  params?: {
    ...
    // P5g1 — multi-output: URLs armazenadas quando a geração produz >1 saída.
    result_urls?: string[];
  } | null;
}
        ...
        <div className="flex max-h-[82vh] items-center justify-center overflow-hidden rounded-xl">
          ...
          {item.type === "audio" && item.result_url && (
            <div className="flex w-[420px] flex-col items-center gap-4 rounded-xl bg-[#1A1A1A] p-8">
              ...
              <MultiAudioPlayer
                urls={getGenerationResultUrls(item)}
                autoPlay
                className="w-full"
              />
            </div>
