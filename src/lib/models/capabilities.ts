...
const VIDEO_CAPS: Record<string, VideoCapabilities> = {
  ...
  // P6d — canônico renomeado IN PLACE: era "seedance-1.5-pro" (id stale). O alias
  // legado resolve para esta MESMA entrada (ver LEGACY_MODEL_ID_ALIASES).
  "seedance-2.0-mini": v(seedance({ resolution: ["480p", "720p"], duration: D.range(4, 15) })),
  ...
  // P6d — canônico renomeado IN PLACE: era "wan-2.1-video" (id stale). O alias
  // legado resolve para esta MESMA entrada (ver LEGACY_MODEL_ID_ALIASES).
  "wan-2.6": v({ startFrame: true, audio: true, negativePrompt: true, wanExtras: true, duration: D.enum([5, 10, 15]), resolution: ["720p", "1080p"], aspectRatio: WAN_ASPECTS }),
};

// P6d — aliases LEGADOS de model_id. O catálogo será renomeado IN PLACE
// (seedance-1.5-pro → seedance-2.0-mini; wan-2.1-video → wan-2.6). Durante a
// transição AMBOS os ids resolvem para a MESMA capability, para o runtime
// funcionar ANTES e DEPOIS da migration (deploy independente da aplicação do DB).
// O provider (task_type) permanece a fonte de verdade do payload — inalterado.
const LEGACY_MODEL_ID_ALIASES: Record<string, string> = {
  "seedance-1.5-pro": "seedance-2.0-mini",
  "wan-2.1-video": "wan-2.6",
};

/** Normaliza um model_id legado para seu id canônico (P6d). Id atual → inalterado. */
...
// P5c — a verdade da UI de Audio vive em `audio-ui-spec.ts` (fonte única rica).
// Aqui não há mais mapa próprio de Audio: `deriveAudio` PROJETA a spec rica para
// os booleans legados desta camada (sem dupla fonte de verdade).
...
function deriveAudio(m: ModelInput): AudioCapabilities {
  // Delegação P5c: projeta a spec rica (audio-ui-spec) nos booleans legados.
  const spec = resolveAudioUISpec({
    modelId: m.model_id,
    kind: m.kind ?? undefined,
    backend: m.backend ?? undefined,
  });
  const legacy = audioUISpecToLegacyCaps(spec);
  return aud({ voice: legacy.voice, lyrics: legacy.lyrics, duration: legacy.duration });
}
...
export function resolveCapabilities(model: ModelInput): Capabilities {
  ...
  // canônico ANTES do lookup, para que rows pré-migration (id antigo no DB) e
  // pós-migration (id novo) resolvam para a MESMA capability.
  if (model.type === "video") return VIDEO_CAPS[canonicalModelId(model.model_id)] ?? deriveVideo(model);
  if (model.type === "image") return IMAGE_CAPS[model.model_id] ?? deriveImage();
  return deriveAudio(model); // Audio: fonte única em audio-ui-spec (delegado)
}
