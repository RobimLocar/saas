...
test("capabilities.test.ts", async () => {
...
// P6d — rename IN PLACE com backward-compat: o id LEGADO (acima, em OLD_VCAPS) e o
// id CANÔNICO novo devem resolver para a MESMA capability, para o runtime funcionar
// antes E depois da migration. Cobre FASE 19 (old resolve / new resolve).
(
  [
  ...
  check(`alias ${oldId}→${newId} resolvem igual`, eq(asVideo(oldId), asVideo(newId)));
  check(`canonical ${newId} tem resolução`, asVideo(newId).resolution.length > 0);
});
